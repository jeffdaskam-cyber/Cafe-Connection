...
function UploadZone({ onUpload, uploadState }) {
  ...
  const icon = isProcessing ? "⏳" : uploadState.status === "SUCCESS" ? "✓" : uploadState.status === "ERROR" ? "✕" : "↑";
  const successMsg = uploadState.campus ? `Filed to ${uploadState.campus}. Drop another.` : "Uploaded! Drop another.";
  const sub = isProcessing ? (uploadState.status === "UPLOADING" ? "Uploading…" : "Analyzing report data…")
  ...
  const accentColor = (uploadState.status === "SUCCESS" && uploadState.campus) ? CAMPUS_COLOR[uploadState.campus] : AQUA;
  ...
  return (
    <div {...getRootProps()} style={{ border:`1.5px dashed ${isDragActive ? accentColor : BORDER}`,
      ...
      <div style={{ color:TPRI, fontSize:13, fontFamily:"'Poppins',sans-serif", fontWeight:600, marginBottom:6 }}>Upload Report</div>
      ...
      {uploadState.status === "SUCCESS" && uploadState.campus && (
        <div style={{ display:"inline-block", marginTop:12, padding:"4px 14px", borderRadius:20,
          background:`${CAMPUS_COLOR[uploadState.campus]}22`, border:`1px solid ${CAMPUS_COLOR[uploadState.campus]}55`,
          color:CAMPUS_COLOR[uploadState.campus], fontSize:11, fontWeight:700,
...
function MonthEndModal({ onClose }) {
  ...
  async function handleGenerate() {
    ...
    try { const data = await getMonthEndData(year, month); setReportData(data); setStatus("done"); }
    catch (err) { console.error(err); setStatus("error"); }
  }

  function buildEmailText() {
    if (!reportData) return "";
    const labels = { "Mesa Lab": "Mesa Lab", "Foothills": "Foothills Cafe", "Center Green": "Center Green" };
    const lines = ["Good morning,", `The month-end information for each cafe is listed below:`, ""];
    CAMPUSES.forEach(c => {
    ...
    lines.push("Please let me know if you have any questions."); lines.push(""); lines.push("Sincerely,");
    return lines.join("\n");
  }

  function handleCopy() {
    navigator.clipboard.writeText(buildEmailText()).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2500); });
  }

  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(1,14,33,0.82)",
      zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(6px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:PANEL, border:`1px solid ${BORDER}`,
        borderRadius:18, padding:"32px 36px", width:"100%", maxWidth:540,
        position:"relative", overflow:"hidden", boxShadow:"0 24px 64px rgba(0,0,0,0.5)" }}>
        <div style={{ position:"absolute", top:0, left:0, right:0, height:3,
          background:`linear-gradient(90deg, ${AQUA}, ${LAQUA}, transparent)`, borderRadius:"18px 18px 0 0" }} />
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
          <div>
            <div style={{ fontSize:15, fontWeight:700, color:TPRI, fontFamily:"'Poppins',sans-serif" }}>Month-End Report</div>
            <div style={{ fontSize:10, color:TSEC, fontWeight:500, letterSpacing:"0.08em", textTransform:"uppercase", marginTop:2 }}>Accounting email generator</div>
          </div>
          <button onClick={onClose} style={{ background:"transparent", border:"none", color:TSEC, fontSize:20, cursor:"pointer" }}>✕</button>
        </div>
        <div style={{ display:"flex", gap:12, marginBottom:24 }}>
          <div style={{ flex:2 }}>
            <div style={{ fontSize:10, color:TSEC, fontWeight:600, letterSpacing:"1.2px", textTransform:"uppercase", marginBottom:8 }}>Month</div>
            <select value={month} onChange={e => setMonth(Number(e.target.value))}
              style={{ width:"100%", background:`${SPACE}cc`, border:`1px solid ${BORDER}`, borderRadius:8,
                color:TPRI, fontFamily:"'Poppins',sans-serif", fontWeight:600, fontSize:12, padding:"9px 12px", cursor:"pointer" }}>
              {MONTH_NAMES.map((m, i) => <option key={m} value={i+1} style={{ background:DARKBLUE }}>{m}</option>)}
            </select>
          </div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:10, color:TSEC, fontWeight:600, letterSpacing:"1.2px", textTransform:"uppercase", marginBottom:8 }}>Year</div>
            <select value={year} onChange={e => setYear(Number(e.target.value))}
              style={{ width:"100%", background:`${SPACE}cc`, border:`1px solid ${BORDER}`, borderRadius:8,
                color:TPRI, fontFamily:"'Poppins',sans-serif", fontWeight:600, fontSize:12, padding:"9px 12px", cursor:"pointer" }}>
              {years.map(y => <option key={y} value={y} style={{ background:DARKBLUE }}>{y}</option>)}
            </select>
          </div>
        </div>
        {status !== "done" && (
          <button onClick={handleGenerate} disabled={status === "loading"}
            style={{ width:"100%", padding:"11px 0", borderRadius:8, border:"none",
              background: status === "loading" ? `${AQUA}88` : AQUA, color:SPACE,
              fontFamily:"'Poppins',sans-serif", fontWeight:700, fontSize:13,
              cursor: status === "loading" ? "default" : "pointer", marginBottom: status === "error" ? 12 : 0 }}>
            {status === "loading" ? "Fetching data…" : `Generate ${monthName} ${year} Report`}
          </button>
        )}
        {status === "error" && (
          <div style={{ color:ORANGE, fontSize:12, fontFamily:"'Poppins',sans-serif", fontWeight:500, textAlign:"center" }}>
        ...
        {status === "done" && (
          <div>
            <div style={{ display:"flex", gap:8, marginBottom:14, flexWrap:"wrap" }}>
              {CAMPUSES.map(c => {
                const src = reportData[c]?.source; const hasData = src && src !== "none"; const cc = CAMPUS_COLOR[c];
                return (
                  <div key={c} style={{ fontSize:10, fontWeight:600, fontFamily:"'Poppins',sans-serif",
                    ...
                    color: hasData ? cc : ORANGE, letterSpacing:"0.04em", textTransform:"uppercase" }}>
            ...
            <textarea readOnly value={buildEmailText()}
              style={{ width:"100%", background:`${SPACE}cc`, border:`1px solid ${BORDER}`,
                ...
                fontSize:12, lineHeight:1.7, padding:"14px 16px", resize:"none",
                outline:"none", boxSizing:"border-box", height:260, marginBottom:12 }} />
            <div style={{ display:"flex", gap:10 }}>
              ...
              <button onClick={() => setStatus("idle")}
                style={{ padding:"10px 18px", borderRadius:8, border:`1px solid ${BORDER}`,
                  background:"transparent", color:TSEC, fontFamily:"'Poppins',sans-serif",
...
export default function FinancialsPage() {
  ...
  const filteredMonthly = period === "monthly" ? monthlyData.filter(d => inFiscalYear(d.monthKey, fiscalYear)) : monthlyData;
  const filteredDaily   = period === "daily"
    ? metrics.filter(d => {
        if (!fiscalYear) return true;
        const dt = d.date?.toDate ? d.date.toDate() : new Date(d.date);
        const m = dt.getMonth()+1, y = dt.getFullYear(), fy = m >= 10 ? y : y-1;
  ...
  return (
    <>
      ...
      <div style={{ maxWidth:1280, margin:"0 auto", padding:"32px 36px" }}>
        ...
        {/* ── Top bar: Campus + FY + Period + Month-End button ── */}
          ...
          {period !== "annual" && fiscalYears.length > 0 && (
            <div>
              ...
              <select value={fiscalYear || ""} onChange={e => setFiscalYear(e.target.value)}
                style={{ background:`${SPACE}cc`, border:`1px solid ${BORDER}`, borderRadius:8, color:TPRI,
                  fontFamily:"'Poppins',sans-serif", fontWeight:600, fontSize:12,
                  padding:"9px 32px 9px 14px", cursor:"pointer", appearance:"none", WebkitAppearance:"none",
                  ...
                  return (
                    <button key={val} onClick={() => setPeriod(val)}
                      style={{ padding:"7px 18px", borderRadius:6, border:"none", cursor:"pointer",
                        fontFamily:"'Poppins',sans-serif", fontWeight:600, fontSize:12, letterSpacing:"0.03em",
        ...
        <div style={{ display:"flex", gap:18, marginBottom:28, animation:"ucar-fadein .5s ease both" }}>
          <StatCard label={period === "daily" ? "Net Revenue (30d)" : period === "monthly" ? "Net Revenue (Monthly)" : "Net Revenue (Annual)"}
            value={`$${(totalSales/1000).toFixed(1)}k`} delta={4.2} accentColor={AQUA} />
          <StatCard label={period === "daily" ? "Avg Daily Checks" : period === "monthly" ? "Avg Monthly Checks" : "Avg Annual Checks"}
            value={avgVolume || "—"} delta={-1.8} accentColor={LAQUA} />
          <StatCard label={period === "daily" ? "Avg Lunch Checks" : "Total Lunch Checks"}
        ...
        {chartData.length > 0 ? (
            ...
            <div style={{ background:PANEL, borderRadius:16, padding:"24px 28px",
              ...
              <ResponsiveContainer width="100%" height={210}>
                <LineChart data={chartData}>
                  ...
                  <XAxis dataKey="date" tick={{ fill:TSEC, fontSize:9, fontFamily:"'Poppins'" }}
                    tickLine={false} axisLine={false} interval={period === "daily" ? 4 : 0} />
                  <YAxis tick={{ fill:TSEC, fontSize:9, fontFamily:"'Poppins'" }} tickLine={false} axisLine={false} />
                  <Tooltip content={<CustomTooltip />} />
          ...
          <div style={{ background:PANEL, border:`1.5px dashed ${BORDER}`, borderRadius:16, padding:56,
            textAlign:"center", marginBottom:28, animation:"ucar-fadein .5s ease both" }}>
            <div style={{ width:56, height:56, borderRadius:"50%", background:`${AQUA}18`,
              border:`1px solid ${AQUA}44`, display:"flex", alignItems:"center",
              justifyContent:"center", margin:"0 auto 16px", fontSize:22 }}>📊</div>
            <div style={{ color:TPRI, fontWeight:700, fontSize:15, marginBottom:6 }}>No {period} data yet for {campus}</div>
            <div style={{ color:TSEC, fontSize:12, fontWeight:500 }}>Upload a report below to populate the charts</div>
          </div>
        )}
        ...
        <div style={{ background:PANEL, borderRadius:16, padding:"26px 28px",
          ...
          <div style={{ fontSize:13, fontWeight:700, marginBottom:2, position:"relative" }}>Upload Reports</div>
