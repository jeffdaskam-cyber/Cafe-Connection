...
import ScheduleNotes   from "./components/ScheduleNotes.jsx";
...
export default function WeeklyOps() {
  ...
  // Schedule: re-fetch whenever weekOf changes
  const {
    data:   scheduleData,
  ...
  // Event orders: real-time subscription (campus-agnostic)
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "32px 36px" }}>
      ...
      {/* ── Row 1: Staff Schedule (full width) ── */}
      <div style={{ marginBottom: 20, animation: "ucar-fadein .5s ease both" }}>
        <Widget
          ...
          empty={!scheduleLoading && !scheduleError && !scheduleData?.rows?.length}
        ...
        >
          {scheduleData && (
            <ScheduleTable
              ...
              colorMap={scheduleData.colorMap}
            />
          )}
        </Widget>
      </div>
