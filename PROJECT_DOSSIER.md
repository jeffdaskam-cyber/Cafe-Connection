...
- **Login**: Firebase magic-link (email link) — no passwords. Existing users request a sign-in link from the LoginPage; invited users receive a one-click sign-in link generated by an administrator.
- **Splash Screen**: Animated branded splash shown once per page load while Firebase auth initializes in the background.
- **Admin-controlled access**: Self-registration is disabled. A user with no `user_roles` record and no matching `pending_invites` document is signed out immediately and shown a "Contact Jeff Daskam to request an invite" panel on the login screen.
- **Roles**: Four tiers stored in Firestore `user_roles` collection (ascending privilege):
  - `user` — read-only access to Dashboard and Weekly Ops
  - `manager` — adds upload zones (DropBox, Event Orders, sales reports) plus Cafe Sales, Event Revenue, and Reports
  - `senior_leader` — Dashboard and FP&A only (executive read view); Weekly Ops/Cafe Sales/Reports hidden
  - `administrator` — full access including the Admin tab, role management, and the FP&A dashboard
- All RBAC is centralized in `src/utils/permissions.js` (`canAccessPage`, `canSeeWidget`, `roleAtLeast`, `ROLES`). Tab-level visibility, widget-level visibility, and read-only widget rendering all derive from this single map.
- Role and invite mutations are routed through the `/api/update-user-role`, `/api/generate-invite-link`, and `/api/delete-invite` serverless endpoints (not direct Firestore writes) to enforce server-side validation.
...
### 5. FP&A *(senior_leader and administrator only)*
...
A two-column workspace at the top of the page (Registered Users on the left, User Invitations on the right), with Vendor Management beneath them.

**Registered Users** — table of every active user from the `user_roles` collection:
- Email address
- Display name *(click-to-edit; merges into both `user_roles` and `users` so the dashboard greeting and the admin table stay in sync)*
- Role dropdown (user / manager / senior_leader / administrator) — changes POST to `/api/update-user-role`
- Last role-assigned date

**User Invitations** — admin-controlled onboarding for new users:
- Create form: `@ucar.edu` email + role (User / Manager / Senior Leader / Administrator). Submit calls `/api/generate-invite-link`, which uses the Firebase Admin SDK to mint a magic-link sign-in URL and writes a `pending_invites/{email}` doc. The client then opens a Gmail compose tab pre-filled with the recipient, subject, body, and the magic-link.
- Pending Invites list (live `onSnapshot` ordered by `invitedAt desc`): email, role, sent date, plus a green "Pending" / amber "Expired" badge derived client-side from `invitedAt + 24h`. Each row has **Resend** (re-mints a fresh link, reopens Gmail, resets `invitedAt`) and **Delete** (calls `/api/delete-invite`).
- Validation: server rejects non-`@ucar.edu` addresses (400), invites for emails already in `user_roles` (409), and any non-administrator caller (403). Client surfaces those errors inline and renders an inline panel if the `pending_invites` subscription itself errors (e.g., un-deployed rules).
- First-login activation: when an invited user clicks their link, `AuthContext` looks up `pending_invites/{email}`, creates `user_roles/{uid}` with the assigned role and a blank `users/{uid}` profile (which triggers the first-run wizard), then deletes the invite.

**Vendor Management** — `vendor_links` CRUD: add, edit, reorder, and delete the vendor tiles shown in the Vendor Portal widget (name, URL, logo URL, sort order).
...
| `/api/generate-invite-link` | POST | Mint a Firebase magic-link for a `@ucar.edu` email and write a `pending_invites` doc (administrator-only). 409 if the email is already in `user_roles`. |
| `/api/delete-invite` | DELETE | Remove a `pending_invites/{email}` document (administrator-only). |
...
| `pending_invites` | Admin-issued user invitations keyed by lowercased email (`email`, `role`, `invitedAt`, `invitedBy`, `status`). Admins read all; the invited user reads/deletes their own doc on first login. All create/update is server-only via `/api/generate-invite-link` and `/api/delete-invite`. |
| `vendor_links` | Vendor portal tile entries (name, URL, logo URL, sort order). Read by any authenticated UCAR user; write restricted to administrators. |
...
### New Feature: Admin-Controlled User Invitations
Self-registration was removed. Administrators now invite users from a new **User Invitations** card on the Admin tab (sits beside Registered Users). The form mints a Firebase magic-link via `/api/generate-invite-link`, writes a `pending_invites/{email}` doc, and opens a Gmail compose tab pre-filled with the recipient, subject, body, and link. The card subscribes to `pending_invites` and shows each invite with a green Pending / amber Expired badge (24-hour expiry derived client-side) plus Resend / Delete actions. Any UCAR-authenticated user with no `user_roles` record and no matching invite is signed out on the spot and shown a "Contact Jeff Daskam to request an invite" panel on the login screen. Firestore rules were tightened in lockstep: `user_roles` self-create now requires a matching `pending_invites` doc (the role must match), and `pending_invites` is admin-readable / server-writable only, with an explicit allow for the invited user to read and delete their own doc during first-login activation.

### New Role: Senior Leader (RBAC Centralized)
A `senior_leader` tier was added between `manager` and `administrator`, scoped to the FP&A dashboard and Dashboard widgets only (no Weekly Ops, Cafe Sales, Event Revenue, or Reports). All permission checks were consolidated into `src/utils/permissions.js` (`ROLES`, `canAccessPage`, `canSeeWidget`, `roleAtLeast`) so tab visibility, widget visibility, and read-only widget rendering all derive from one map. The Admin tab role dropdown picked up the new tier automatically.

### Vendor Management on the Admin Tab
Administrators can now CRUD vendor portal tiles (name, URL, logo URL, sort order) directly from the Admin tab. Records live in the new `vendor_links` collection: read by any authenticated UCAR user, write restricted to administrators via Firestore rules. The Vendor Portal widget reads from this collection live.
